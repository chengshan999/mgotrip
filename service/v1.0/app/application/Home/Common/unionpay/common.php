...
include_once C('UNIONPAY_FILE_PATH') . 'secureUtil.php';
...
function deflate_file(&$params) {
	foreach ( $_FILES as $file ) {
		...
		if (file_exists ( $file ['tmp_name'] )) {
			...
			\Think\Log::write("压缩后文件内容为>" . base64_encode ( $file_content_deflate ),'UNIONPAY');
		} else {
			\Think\Log::write(">>>>文件上传失败<<<<<",'UNIONPAY');
		}
	}
}
...
function deal_file($params) {
	if (isset ( $params ['fileContent'] )) {
		\Think\Log::write("---------处理后台报文返回的文件---------",'UNIONPAY');
		$fileContent = $params ['fileContent'];
		
		if (empty ( $fileContent )) {
			\Think\Log::write('文件内容为空','UNIONPAY');
			return false;
		} else {
			// 文件内容 解压缩
			...
			if (empty ( $params ['fileName'] )) {
				\Think\Log::write('文件名为空','UNIONPAY');
			...
			if (! is_writable ( $filePath )) {
				\Think\Log::write("文件:" . $filePath . "不可写，请检查！",'UNIONPAY');
				return false;
			} else {
				file_put_contents ( $filePath, $content );
				\Think\Log::write("文件位置 >:" . $filePath,'UNIONPAY');
			}
			fclose ( $handle );
		}
		return true;

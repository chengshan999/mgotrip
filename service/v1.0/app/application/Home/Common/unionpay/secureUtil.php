<?php
include_once C('UNIONPAY_FILE_PATH') . 'SDKConfig_offical.php';
...
function sign(&$params) {
	\Think\Log::write('=====签名报文开始======','UNIONPAY');
	if(isset($params['signature'])){
		unset($params['signature']);
	}
	// 转换成key=val&串
	$params_str = createLinkString ( $params, true, false );
	\Think\Log::write("签名key=val&...串 >" . $params_str,'UNIONPAY');
	
	$params_sha1x16 = sha1 ( $params_str, FALSE );
	\Think\Log::write("摘要sha1x16 >" . $params_sha1x16,'UNIONPAY');
	...
	if ($sign_falg) {
		...
		\Think\Log::write("签名串为 >" . $signature_base64,'UNIONPAY');
		$params ['signature'] = $signature_base64;
	} else {
		\Think\Log::write(">>>>>签名失败<<<<<<<",'UNIONPAY');
	}
	\Think\Log::write('=====签名报文结束======','UNIONPAY');
}
...
//	echo $public_key.'<br/>';
	...
	unset ( $params ['signature'] );
	$params_str = createLinkString ( $params, true, false );
	\Think\Log::write('报文去[signature] key=val&串>' . $params_str,'UNIONPAY');
	$signature = base64_decode ( $signature_str );
//	echo date('Y-m-d',time());
	$params_sha1x16 = sha1 ( $params_str, FALSE );
	\Think\Log::write('摘要shax16>' . $params_sha1x16,'UNIONPAY');
	$isSuccess = openssl_verify ( $params_sha1x16, $signature,$public_key, OPENSSL_ALGO_SHA1 );
	\Think\Log::write($isSuccess ? '验签成功' : '验签失败','UNIONPAY');
	return $isSuccess;
}
...
function getPulbicKeyByCertId($certId) {
	\Think\Log::write('报文返回的证书ID>' . $certId,'UNIONPAY');
	// 证书目录
	$cert_dir = SDK_VERIFY_CERT_DIR;
	\Think\Log::write('验证签名证书目录 :>' . $cert_dir,'UNIONPAY');
			...
			if (is_file ( $filePath )) {
				if (pathinfo ( $file, PATHINFO_EXTENSION ) == 'cer') {
					if (getCertIdByCerPath ( $filePath ) == $certId) {
						closedir ( $handle );
						\Think\Log::write('加载验签证书成功','UNIONPAY');
						return getPublicKey ( $filePath );
					}
				}
			}
		}
		\Think\Log::write('没有找到证书ID为[' . $certId . ']的证书','UNIONPAY');
	} else {
		\Think\Log::write('证书目录 ' . $cert_dir . '不正确','UNIONPAY');
	}
	closedir ( $handle );
	return null;
}
